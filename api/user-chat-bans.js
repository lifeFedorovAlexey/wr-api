import { banChatMember } from "../lib/chatModeration.mjs";
...
export default async function handler(req, res) {
  ...
  try {
    const ban = await banChatMember(session.user.id, req.body || {});
    return res.status(200).json({ ban });
  } catch (error) {
    const code = error instanceof Error ? error.message : "chat_ban_failed";
