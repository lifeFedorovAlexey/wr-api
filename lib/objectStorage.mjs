...
async function ensureExpirationLifecycleRule(client, config, { id, prefix, days }) {
  let currentRules = [];
  try {
    ...
    if (statusCode !== 404 && name !== "NoSuchLifecycleConfiguration") throw error;
  }
  ...
  await client.send(
    new PutBucketLifecycleConfigurationCommand({
      ...
      LifecycleConfiguration: {
        Rules: [...currentRules.filter((rule) => rule?.ID !== nextRule.ID), nextRule],
      },
    }),
  );
  return nextRule;
...
async function ensureCorsRule(client, config, { id, allowedOrigins }) {
  ...
  const nextRule = {
    ID: String(id),
    AllowedOrigins: Array.from(new Set(allowedOrigins.map(String).map((value) => value.trim()).filter(Boolean))),
  ...
  await client.send(
    new PutBucketCorsCommand({
      Bucket: config.bucket,
      CORSConfiguration: {
        CORSRules: [...currentRules.filter((rule) => rule?.ID !== nextRule.ID), nextRule],
      },
    }),
  );
  return nextRule;
...
export function createObjectStorageClient(env = process.env) {
  ...
  async function deleteObjects(keys = []) {
    const normalizedKeys = Array.from(
      new Set(keys.map((key) => String(key || "").replace(/^\/+/, "")).filter(Boolean)),
    );
    ...
    for (let offset = 0; offset < normalizedKeys.length; offset += 1_000) {
      await client.send(
        new DeleteObjectsCommand({
          ...
          Delete: {
            Quiet: true,
            Objects: normalizedKeys.slice(offset, offset + 1_000).map((Key) => ({ Key })),
          },
        }),
      );
    }
