...
import { createGuideAssetStore } from "./guideAssets.mjs";
import { buildGuideImportRecord, collectGuideAssetTasks, shouldSkipGuideImport } from "./guides.mjs";
import { createObjectStorageClient, shouldUseS3PublicUrls } from "./objectStorage.mjs";
...
async function mirrorGuideImportAssets(record, env = process.env) {
  const tasks = collectGuideAssetTasks(record);
  if (!tasks.length) {
    return;
  }

  const requireStrictMirror = shouldUseS3PublicUrls(env);
  const objectStorage = createObjectStorageClient(env);

  if (requireStrictMirror && !objectStorage) {
    throw new Error("Guide asset mirror requires configured S3 credentials in s3 public mode");
  }

  const guideAssetStore = await createGuideAssetStore(env, {
    objectStorage,
    throwOnMirrorError: requireStrictMirror,
  });

  for (const task of tasks) {
    await guideAssetStore.mirror(task.assetKey, task.sourceUrl);
  }
}
...
export async function importGuidePayload(guide, options = {}) {
  ...
  await mirrorGuideImportAssets(record, options.env || process.env);
