...
export function buildGuideHeroMediaStorageKey(slug) {
  return buildStorageKey("hero-media", buildGuideHeroMediaFileName(slug));
}

export function buildPublicGuideHeroMediaPath(slug, env = process.env) {
  if (shouldUseS3PublicUrls(env)) {
    return (
      buildObjectStoragePublicUrl(buildGuideHeroMediaStorageKey(slug), env) ||
      `/wr-api/hero-media/${buildGuideHeroMediaFileName(slug)}`
    );
  }
