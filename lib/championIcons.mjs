...
export function buildPublicIconPath(slug, sourceUrl = null) {
  const qs = sourceUrl ? `?src=${encodeURIComponent(sourceUrl)}` : "";
  return `/wr-api/icons/${slug}${qs}`;
}
...
export async function createChampionIconStore(env = process.env) {
  ...
  async function mirror(slug, sourceUrl) {
    ...
    if (
      ...
      return buildPublicIconPath(slug, sourceUrl);
    }
  ...
  return { mirror, iconsDir, getCachedFilePath };
}
