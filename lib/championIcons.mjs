...
export function buildPublicIconPath(slug, sourceUrl = null, env = process.env, size = null) {
  ...
  if (shouldUseS3PublicUrls(env) && sourceUrl) {
    const storageKey = normalizedSize
      ...
      : buildIconStorageKey(slug, sourceUrl);

    return buildObjectStoragePublicUrl(storageKey, env) || buildLocalIconPath(slug, sourceUrl, normalizedSize);
  }

  return buildLocalIconPath(slug, sourceUrl, normalizedSize);
}
