...
export async function listQuizzes(actor, { managed = false } = {}) {
  ...
  for (const row of rows) {
    if (
      ...
      const quiz = toQuizView(row);
      if (managed) quiz.capabilities = { delete: canDeleteQuiz(actor, row) };
      result.push(quiz);
    }
  }
  return result;
...
export async function changeQuizStatus(actor, quizId, action, reason = "") {
  const row = await loadQuizRow(quizId);
  const adminActions = new Set(["block", "restore"]);
  if (action === "delete") {
    if (!actor)
      throw appError("unauthorized", 401);
    if (!canDeleteQuiz(actor, row))
      throw appError("forbidden", 403);
  } else if (adminActions.has(action))
    requireQuizCapability(
      actor,
      action === "block"
