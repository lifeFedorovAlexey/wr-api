...
import { mergeScrapedChampionWithExistingRow } from "./championScrapeMerge.mjs";
...
export async function updateChampions() {
  ...
  const existingRowsBefore = await db
    ...
    .from(champions);
  const existingChampionMap = new Map(
    existingRowsBefore
  ...
  for (const c of items) {
    const existingRow = existingChampionMap.get(String(c?.slug || "").trim()) || null;
    const mergedChampion = mergeScrapedChampionWithExistingRow(c, existingRow);

    if (mergedChampion?.slug) {
      scrapedSlugs.add(mergedChampion.slug);
    }

    const names = mergedChampion.names || {};
    ...
    const rolesLocalizations = mergedChampion.rolesLocalized || {
      ru_ru: [],
      en_us: [],
      zh_cn: [],
    };

    const difficultyLocalizations = mergedChampion.difficultyLocalized || {
      ru_ru: null,
      en_us: null,
      zh_cn: null,
    };
    ...
    if (validation.missing.length > 0) {
      problems.push({
        slug: mergedChampion.slug || "(no-slug)",
    ...
    const rowToInsert = {
      slug: mergedChampion.slug,
      cnHeroId: mergedChampion.cnHeroId || null,
      nameLocalizations,
      name: baseName,
      roles: mergedChampion.roles || [],
      difficulty: mergedChampion.difficulty || null,
      icon: mergedChampion.icon || null,
      rolesLocalizations,
      difficultyLocalizations,
    };

    if (existingSlugSet.has(mergedChampion.slug)) {
      updated += 1;
    } else {
      created += 1;
    }
