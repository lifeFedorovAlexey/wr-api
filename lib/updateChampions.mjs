...
import { createChampionIconStore } from "./championIcons.mjs";
...
export async function updateChampions() {
  ...
  const iconStore = await createChampionIconStore();
  ...
  for (const c of items) {
    ...
    const localIconPath = await iconStore.mirror(c.slug, c.icon);
