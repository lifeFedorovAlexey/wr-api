...
export async function listLatestStreamerTierlists() {
  ...
  const [users, accessUsers] = await Promise.all([db.select().from(siteUsers), listAccessUsers()]);
...
export async function publishPublicTierlist(
  ...
  const authorName = normalizeTierlistAuthorName(
    ...
    normalizeTierlistAuthorName(siteUser?.streamerDisplayName || siteUser?.displayName),
  );
  const now = new Date();
