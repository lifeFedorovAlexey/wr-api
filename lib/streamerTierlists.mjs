...
function createEmptyPublishedPayload(mode = "lanes") {
  ...
  return {
    ...
    tierStyles: Object.fromEntries(
      STREAMER_TIERLIST_TIERS.map((tier) => [
        tier,
        { label: tier, color: DEFAULT_TIER_COLORS[tier] },
      ]),
    ),
...
function sanitizeTierStyles(value) {
...
function sanitizePublicationPayload(rawPayload) {
  ...
  payload.tierStyles = sanitizeTierStyles(rawPayload?.tierStyles);
  const sourceLanes =
...
export function sanitizeStreamerTierlistSubmission(input, championMap) {
  ...
  payload.tierStyles = sanitizeTierStyles(input?.tierStyles);
  const sourceLanes =
