...
const MAX_STREAMER_TIERLIST_TIERS = 12;
...
const DEFAULT_TIER_COLORS = Object.freeze({
  "S+": "#b78aff",
  S: "#f19797",
  A: "#f0c58b",
  B: "#f3f59a",
...
export const PUBLIC_TIERLIST_ANONYMOUS_TTL_MS = 30 * 24 * 60 * 60 * 1000;
...
function normalizeTierKey(value) {
  const tierKey = String(value || "").trim().slice(0, 40);
  return /^[A-Za-z0-9+_-]+$/.test(tierKey) ? tierKey : null;
}

function sanitizeTierOrder(value) {
  const source = Array.isArray(value) ? value : STREAMER_TIERLIST_TIERS;
  const seen = new Set();
  const tiers = [];
  ...
  for (const value of source) {
    ...
    seen.add(tier);
    tiers.push(tier);
    if (tiers.length >= MAX_STREAMER_TIERLIST_TIERS) break;
  }

  return tiers.length ? tiers : [...STREAMER_TIERLIST_TIERS];
}
...
export function isAnonymousPublicTierlistExpired(row, now = new Date()) {
  if (!row || row.siteUserId) return false;
  const publishedAt = row.publishedAt instanceof Date ? row.publishedAt : new Date(row.publishedAt || 0);
  if (Number.isNaN(publishedAt.getTime())) return false;
  return now.getTime() - publishedAt.getTime() >= PUBLIC_TIERLIST_ANONYMOUS_TTL_MS;
}

export function userCanDeleteStreamerTierlist(user, row) {
  if (!user?.id || !row) return false;
  if (userHasRole(user, ["owner", "admin"])) return true;
  return Boolean(row.siteUserId) && Number(row.siteUserId) === Number(user.id);
}
...
function createEmptyTierBuckets(tiersOrder = STREAMER_TIERLIST_TIERS) {
  return Object.fromEntries(tiersOrder.map((tier) => [tier, []]));
}
...
function createEmptyPublishedPayload(mode = "lanes", tiersOrderInput = STREAMER_TIERLIST_TIERS) {
  const normalizedMode = normalizeTierlistMode(mode);
  const tiersOrder = sanitizeTierOrder(tiersOrderInput);
  const boardKeys =
  ...
  return {
    ...
    mode: normalizedMode,
    tiersOrder,
    tierStyles: Object.fromEntries(
      tiersOrder.map((tier) => [
        tier,
        { label: tier, color: DEFAULT_TIER_COLORS[tier] || "#94a3b8" },
      ]),
    ),
    lanes: Object.fromEntries(
      boardKeys.map((lane) => [
        ...
        {
          lane,
          tiers: createEmptyTierBuckets(tiersOrder),
        },
      ]),
    ),
  };
...
function sanitizeTierStyles(value, tiersOrder = STREAMER_TIERLIST_TIERS) {
  const source = value && typeof value === "object" ? value : {};
  return Object.fromEntries(
    tiersOrder.map((tier) => {
      const custom = source[tier] && typeof source[tier] === "object" ? source[tier] : {};
      const label = String(custom.label || "").trim().slice(0, 150) || tier;
      const color = /^#[0-9a-f]{6}$/i.test(String(custom.color || ""))
        ? String(custom.color).toLowerCase()
        : DEFAULT_TIER_COLORS[tier] || "#94a3b8";
...
function sanitizePublicationPayload(rawPayload) {
  const payload = createEmptyPublishedPayload(rawPayload?.mode, rawPayload?.tiersOrder);
  payload.tierStyles = sanitizeTierStyles(rawPayload?.tierStyles, payload.tiersOrder);
  const sourceLanes =
  ...
  for (const lane of Object.keys(payload.lanes)) {
    ...
    for (const tier of payload.tiersOrder) {
...
export function sanitizeStreamerTierlistSubmission(input, championMap) {
  const payload = createEmptyPublishedPayload(input?.mode, input?.tiersOrder);
  payload.tierStyles = sanitizeTierStyles(input?.tierStyles, payload.tiersOrder);
  const sourceLanes =
...
export async function cleanupExpiredAnonymousPublicTierlists(now = new Date()) {
  const expiresBefore = new Date(now.getTime() - PUBLIC_TIERLIST_ANONYMOUS_TTL_MS);
  const deleted = await db
    .delete(streamerTierlistPublications)
    .where(
      and(
        isNull(streamerTierlistPublications.siteUserId),
        lte(streamerTierlistPublications.publishedAt, expiresBefore),
      ),
    )
    .returning({ id: streamerTierlistPublications.id });
...
export async function loadStreamerTierlistEditor(siteUserId, { lang = "ru_ru", env = process.env } = {}) {
  ...
  return {
    ...
    sourceSnapshot: latestSnapshot
      ...
      : null,
    tiersOrder: currentPublication?.payload?.tiersOrder || [...STREAMER_TIERLIST_TIERS],
...
export async function deleteStreamerTierlist(actor, { publicId = null, siteUserId = null } = {}) {
  if (!actor?.id) throw new Error("unauthorized");
  const normalizedPublicId = publicId ? normalizePublicTierlistId(publicId) : null;
  const normalizedSiteUserId = normalizeSiteUserId(siteUserId || actor.id);
  if (publicId && !normalizedPublicId) throw new Error("invalid_public_id");
  if (!normalizedPublicId && !normalizedSiteUserId) throw new Error("invalid_site_user");

  const rows = normalizedPublicId
    ? await fetchPublicationRowsForPublicId(normalizedPublicId, 100)
    : await fetchPublicationRowsForSiteUser(normalizedSiteUserId, 100);
  if (!rows.length) throw new Error("tierlist_not_found");
  const latestRow = rows[0];
  if (!userCanDeleteStreamerTierlist(actor, latestRow)) throw new Error("forbidden");

  const deleted = await db
    .delete(streamerTierlistPublications)
    .where(
      normalizedPublicId
        ? eq(streamerTierlistPublications.publicId, normalizedPublicId)
        : eq(streamerTierlistPublications.siteUserId, normalizedSiteUserId),
    )
    .returning({ id: streamerTierlistPublications.id });

  return {
    deleted: deleted.length,
    publicId: normalizedPublicId || latestRow.publicId || null,
    siteUserId: latestRow.siteUserId || null,
  };
}
...
export async function listAllTierlistsForAdmin() {
  const publicationRows = await db
    .select()
    .from(streamerTierlistPublications)
    .orderBy(
  ...
  for (const row of publicationRows) {
    const publicId = row.publicId || (row.siteUserId ? `streamer-${row.siteUserId}` : null);
    if (!publicId || seenPublicIds.has(publicId)) continue;
    seenPublicIds.add(publicId);
    latestRows.push(row);
  }

  const siteUserIds = [...new Set(latestRows.map((row) => row.siteUserId).filter(Boolean))];
  const users = siteUserIds.length
    ? await db.select().from(siteUsers).where(inArray(siteUsers.id, siteUserIds))
    : [];
  const usersById = new Map(users.map((user) => [user.id, user]));

  return latestRows.map((row) => {
    const user = row.siteUserId ? usersById.get(row.siteUserId) : null;
    return {
      publication: toPublicationView(row),
      creator: user
        ? {
            id: user.id,
            displayName: String(user.displayName || user.streamerDisplayName || "").trim() || `User #${user.id}`,
          }
        : null,
      creatorType: row.siteUserId ? "authenticated" : "anonymous",
    };
  });
}
...
export async function loadPublicTierlistEditor(
  ...
  if (String(publicId || "").trim() || String(editToken || "").trim()) {
    throw new Error("public_tierlist_edit_forbidden");
  }
  const normalizedPublicId = publicId ? normalizePublicTierlistId(publicId) : null;
  ...
  return {
    ...
    sourceSnapshot: latestSnapshot
      ...
      : null,
    tiersOrder: latestRow
      ? toPublicationView(latestRow).payload.tiersOrder
...
export async function publishPublicTierlist(
  submission,
  { editToken = null, lang = "ru_ru", env = process.env } = {},
) {
  if (String(submission?.publicId || "").trim() || String(editToken || "").trim()) {
    throw new Error("public_tierlist_edit_forbidden");
  }
  const credentials = createPublicTierlistCredentials();
  const publicId = credentials.publicId;
  const [latestSnapshot, championsCatalog] = await Promise.all([
    getLatestCompletedChampionStatsSnapshot(),
    buildStreamerChampionCatalog(lang, env),
  ]);
...
export async function getPublicTierlistById(publicId, { historyLimit = 8 } = {}) {
  ...
  if (isAnonymousPublicTierlistExpired(latest)) {
