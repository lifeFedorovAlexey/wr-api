...
export function buildPublicSkinAssetPath(championSlug, skinName, kind, sourceUrl = null, env = process.env) {
...
export function normalizeSkinAssetPath(championSlug, skinName, kind, sourceUrl, env = process.env) {
  ...
  try {
    ...
    if (url.protocol === "http:" || url.protocol === "https:") {
      return buildPublicSkinAssetPath(championSlug, skinName, kind, sourceUrl, env);
    }
  } catch {}
...
export function toSkinDto(row, env = process.env) {
  const imageUrl = normalizeSkinAssetPath(
    row.championSlug,
    row.skinName,
    "image",
    row.imageSourceUrl || row.imageAssetPath || null,
    env,
  );
  const modelUrl = normalizeSkinAssetPath(
    row.championSlug,
    row.skinName,
    "model",
    row.modelSourceUrl || row.modelAssetPath || null,
    env,
  );
...
export function assembleSkinCollection(collection, rows, env = process.env) {
  const sortedRows = [...rows].sort((left, right) => left.sortOrder - right.sortOrder);
  const skins = sortedRows.map((row) => toSkinDto(row, env));
