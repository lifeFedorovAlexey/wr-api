...
function extractJsonValue(source, key) {
  return extractJsonValues(source, key)[0] || null;
}
...
function countRiftGgStatsEntries(stats = {}) {
  return ["matchups", "core_items", "runes", "spells"].reduce(
    (total, key) => total + (Array.isArray(stats?.[key]) ? stats[key].length : 0),
    0,
  );
}
...
function pickBestStatsCandidate(candidates = []) {
  return candidates
    .slice()
    .sort((left, right) => countRiftGgStatsEntries(right) - countRiftGgStatsEntries(left))[0] || null;
}
...
export function parseRiftGgCnStatsHtml(html = "") {
  ...
  const itemsDict = parseJsonCandidates(decodedPayload, "itemsDict")[0] || null;
  const runesDict = parseJsonCandidates(decodedPayload, "runesDict")[0] || null;
  const spellsDict = parseJsonCandidates(decodedPayload, "spellsDict")[0] || null;
