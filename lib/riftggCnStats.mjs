...
export function parseRiftGgCnStatsHtml(html = "") {
  ...
  const stats = pickBestStatsCandidate([
    tabsPayload?.stats || null,
    ...parseJsonCandidates(decodedPayload, "stats"),
  ]);
  const itemsDict = tabsPayload?.itemsDict || parseJsonCandidates(decodedPayload, "itemsDict")[0] || null;
  const runesDict = tabsPayload?.runesDict || parseJsonCandidates(decodedPayload, "runesDict")[0] || null;
  const spellsDict = tabsPayload?.spellsDict || parseJsonCandidates(decodedPayload, "spellsDict")[0] || null;
