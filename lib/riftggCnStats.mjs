...
export function decodeFlightPayload(html = "") {
  const chunks = [];
...
export function parseJsonCandidates(source, key) {
...
function looksLikeStatsPayload(candidate) {
  return Boolean(
    candidate &&
      typeof candidate === "object" &&
      (
        Array.isArray(candidate.matchups) ||
        Array.isArray(candidate.core_items) ||
        Array.isArray(candidate.runes) ||
...
export function extractCnStatsTabsPayload(source) {
...
export function extractCompositeStatsPayloads(source) {
  const payloads = [];
  let searchIndex = 0;
...
export function countUsableStatsEntries(stats = {}) {
  let total = 0;
...
function pickBestStatsCandidate(candidates = []) {
  return candidates
    .filter(looksLikeStatsPayload)
    .slice()
    .sort((left, right) => {
      const usableDelta = countUsableStatsEntries(right) - countUsableStatsEntries(left);
      if (usableDelta !== 0) return usableDelta;

      const collectionDelta =
        (countCollectionEntries(right.matchups) +
          countCollectionEntries(right.core_items) +
          countCollectionEntries(right.runes) +
          countCollectionEntries(right.spells)) -
        (countCollectionEntries(left.matchups) +
          countCollectionEntries(left.core_items) +
          countCollectionEntries(left.runes) +
          countCollectionEntries(left.spells));
...
function collectReferencedDictionarySlugs(stats = {}, key) {
  const slugs = new Set();

  const collections = [
    { rows: stats.core_items, buildKey: "builds", entryKey: "items" },
    { rows: stats.runes, buildKey: "builds", entryKey: "runes" },
    { rows: stats.spells, buildKey: "spells", entryKey: "spells" },
  ];

  for (const collection of collections) {
    if (collection.entryKey !== key) continue;

    for (const row of Array.isArray(collection.rows) ? collection.rows : []) {
      const entries = Array.isArray(row?.[collection.buildKey]) ? row[collection.buildKey] : [];
      for (const entry of entries) {
        const entryItems = Array.isArray(entry?.[key]) ? entry[key] : [];
...
function pickBestDictionaryCandidate(candidates = [], referencedSlugs = new Set()) {
  return candidates
    ...
    .sort((left, right) => {
      ...
      const leftCoverage = leftKeys.filter((slug) => referencedSlugs.has(slug)).length;
      const rightCoverage = rightKeys.filter((slug) => referencedSlugs.has(slug)).length;
...
export function hasRequiredDictionaries(payload) {
...
export function parseRiftGgCnStatsHtml(html = "") {
  ...
  const stats = pickBestStatsCandidate(parseJsonCandidates(decodedPayload, "stats"));
  const itemsDict = pickBestDictionaryCandidate(
    parseJsonCandidates(decodedPayload, "itemsDict"),
    collectReferencedDictionarySlugs(stats, "items"),
  );
  const runesDict = pickBestDictionaryCandidate(
    parseJsonCandidates(decodedPayload, "runesDict"),
    collectReferencedDictionarySlugs(stats, "runes"),
  );
  const spellsDict = pickBestDictionaryCandidate(
    parseJsonCandidates(decodedPayload, "spellsDict"),
    collectReferencedDictionarySlugs(stats, "spells"),
  );
...
function pickLatestRowsPerFilter(rows = [], getFilterKey = (row) => `${row.rank}::${row.lane}`) {
  const latestDateByFilter = new Map();

  for (const row of rows) {
    const key = getFilterKey(row);
    const current = latestDateByFilter.get(key) || null;
    const candidate = row?.dataDate || null;

    if (!current || (candidate && candidate > current)) {
      latestDateByFilter.set(key, candidate);
    }
  }

  return rows.filter((row) => {
    const key = getFilterKey(row);
    const latestDate = latestDateByFilter.get(key) || null;
    return (row?.dataDate || null) === latestDate;
  });
}
...
export function buildRiftGgGuidePayload({
  ...
  const latestMatchupRows = pickLatestRowsPerFilter(filteredMatchupRows);
  const latestBuildRows = pickLatestRowsPerFilter(
    filteredBuildRows,
    (row) => `${row.rank}::${row.lane}::${row.buildType || ""}`,
  );

  if (!latestMatchupRows.length && !latestBuildRows.length) {
    return null;
  }
  ...
  const groupedMatchups = groupByFilter(latestMatchupRows).map((group) => {
  ...
  const groupedBuilds = (buildType) =>
    groupByFilter(latestBuildRows.filter((row) => row.buildType === buildType)).map((group) => ({
      rank: group.rank,
      lane: group.lane,
      dataDate: group.dataDate,
  ...
  for (const row of [...latestMatchupRows, ...latestBuildRows]) {
