...
function normalizeGuideAssetPath(assetKey, sourceUrl) {
  ...
  try {
    ...
    // Keep non-URL values untouched so callers can still use stored local paths.
  }

  return sourceUrl;
}

function isRemoteHttpAssetUrl(value) {
  try {
...
export function shouldSkipGuideImport(existingSummary, nextSummary) {
  const existingHash = String(existingSummary?.contentHash || "").trim();
  const nextHash = String(nextSummary?.contentHash || "").trim();
...
export function localizeRole(value = "") {
  ...
  if (!normalized || containsGuideArchetypeTerm(normalized)) return "";
  ...
  if (normalized.includes("top") || normalized.includes("solo")) return "Барон";
  ...
  if (normalized.includes("adc") || normalized.includes("адк")) return "Дракон";
...
export function collectGuideEntityRefs({
  abilities = [],
  buildBreakdown = null,
  sections = [],
  skillOrders = [],
  ...
  for (const row of sections) {
    ...
    const kind =
      sectionType === "itemBuild" || sectionType === "situationalItems"
        ? "item"
        : sectionType === "spellsAndRunes" && row?.sectionKey === "summonerSpells"
          ? "summonerSpell"
          : sectionType === "spellsAndRunes" || sectionType === "situationalRunes"
            ? "rune"
            : null;

    for (const entitySlug of row?.entitySlugs || []) {
      push(kind, entitySlug);
    }
  }

  for (const row of skillOrders) {
    for (const entitySlug of row?.quickOrder || []) {
      push("ability", entitySlug);
    }
  }

  for (const row of skillRows) {
    push("ability", row?.abilitySlug);
  }
  ...
  for (const entitySlug of buildBreakdown?.featuredItemSlugs || []) {
    push("item", entitySlug);
  }

  for (const row of matchups) {
    push("champion", row?.championSlug);
  }

  return Array.from(refs.values());
}

export function collectGuideAssetTasks({
  summary,
  entities = [],
  abilities = [],
}) {
  const guideSlug = String(summary?.slug || "").trim();
  const tasks = [];
  const seen = new Set();

  const push = (assetKey, sourceUrl, meta = {}) => {
    const normalizedSourceUrl = String(sourceUrl || "").trim();
    if (!assetKey || !normalizedSourceUrl || !isRemoteHttpAssetUrl(normalizedSourceUrl)) {
      return;
    }

    const dedupeKey = `${assetKey}::${normalizedSourceUrl}`;
  ...
  for (const entity of entities) {
    ...
    if (kind === "ability" || kind === "champion") continue;

    push(
      buildGuideAssetKey("guide", kind, slug, "image"),
      entity?.imageUrl,
      { kind, slug, field: "image" },
    );
    push(
      buildGuideAssetKey("guide", kind, slug, "tooltip"),
      entity?.tooltipImageUrl,
      { kind, slug, field: "tooltip" },
    );
  }

  if (guideSlug) {
    for (const ability of abilities) {
      const abilitySlug = String(ability?.abilitySlug || ability?.slug || "").trim();
      if (!abilitySlug) continue;

      push(
        buildGuideAssetKey("guide", guideSlug, abilitySlug, "ability"),
        ability?.iconUrl,
        { kind: "ability", slug: abilitySlug, field: "abilityIcon", guideSlug },
      );
    }
  }
...
export function assembleGuideDetail({
  ...
  const resolvedHeroLocalVideoPath =
    ...
    (officialMeta?.heroRemoteVideoUrl && summary?.slug
