...
export async function createGuideAssetStore(env = process.env, options = {}) {
  ...
  async function mirror(assetKey, sourceUrl) {
    ...
    const prevFilePath = prev?.fileName ? path.join(assetsDir, prev.fileName) : null;
    if (
      prev?.sourceUrl === sourceUrl &&
      prev?.fileName &&
      prevFilePath &&
      existsSync(prevFilePath)
    ) {
    ...
    try {
      ...
      await ensureObjectStorageMirror(safeKey, sourceUrl, finalPath, contentType);
