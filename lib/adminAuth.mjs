  ...
  return siteUsersWithIdentities
    .map(({ user, identities }) => {
      ...
      return {
        siteUserId: user.id,
        displayName: user.displayName || "",
        streamerDisplayName: user.streamerDisplayName || "",
...
export async function updateSiteUserAccessRoles(siteUserIdInput, roleKeysInput, options = {}) {
  await ensureRoleSeeds();
  ...
  const streamerDisplayName = normalizeStreamerDisplayName(options.streamerDisplayName);
  ...
  await db.transaction(async (tx) => {
    ...
    if (!adminUserId && roleKeys.length === 0) {
      await tx
        .update(siteUsers)
        .set({
          streamerDisplayName: streamerDisplayName || null,
          updatedAt: new Date(),
        })
