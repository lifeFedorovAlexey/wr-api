...
export const COMPLETED_SNAPSHOT_COVERAGE_RATIO = 0.95;
export const MAX_MISSING_CHAMPIONS_FOR_COMPLETED = 1;
...
export function determineChampionStatsSnapshotStatus({
  ...
  const totalChampions = Number(championCount) || 0;
  const matchedChampions = Number(matchedChampionCount) || 0;
  ...
  const missingChampions = Math.max(totalChampions - matchedChampions, 0);
  const coverage = matchedChampions / totalChampions;

  return (
    missingChampions <= MAX_MISSING_CHAMPIONS_FOR_COMPLETED ||
    coverage >= COMPLETED_SNAPSHOT_COVERAGE_RATIO
  )
...
export async function getCurrentChampionStatsSnapshot() {
  const [row] = await db
    .select()
    .from(championStatsSnapshots)
    .where(eq(championStatsSnapshots.source, CHAMPION_STATS_SNAPSHOT_SOURCE))
    .orderBy(desc(championStatsSnapshots.startedAt), desc(championStatsSnapshots.id))
    .limit(1);
